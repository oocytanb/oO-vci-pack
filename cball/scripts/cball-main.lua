...
CreateSlideSwitch=function(cU)local cV=a.NillableValue(cU.colliderItem)local cW=a.NillableValue(cU.baseItem)local cX=a.NillableValue(cU.knobItem)local cY=a.NillableValueOrDefault(cU.minValue,0)local cZ=a.NillableValueOrDefault(cU.maxValue,10)if cY>=cZ then error('SlideSwitch: Invalid argument: minValue >= maxValue',2)end;local c_=cZ-cY;local d0=function(aA)local d1,d2=a.PingPong(aA-cY,c_)return d1+cY,d2 end;local q=d0(a.NillableValueOrDefault(cU.value,0))local d3=a.NillableIfHasValueOrElse(cU.tickFrequency,function(d4)if d4<=0 then error('SlideSwitch: Invalid argument: tickFrequency <= 0',3)end;return math.min(d4,c_)end,function()return c_/10.0 end)local d5=a.NillableIfHasValueOrElse(cU.tickVector,function(b0)return Vector3.__new(b0.x,b0.y,b0.z)end,function()return Vector3.__new(0.01,0.0,0.0)end)local d6=d5.magnitude;if d6<Vector3.kEpsilon then error('SlideSwitch: Invalid argument: tickVector is too small',2)end;local d7=a.NillableValueOrDefault(cU.snapToTick,true)local d8=TimeSpan.FromMilliseconds(1000)local d9=TimeSpan.FromMilliseconds(50)local da,db;local cf={}local self;local dc=false;local dd=false;local de=TimeSpan.Zero;local df=TimeSpan.Zero;local dg=Vector3.zero;local dh=function()local di=d0(da())if di~=q then q=di;for ch,E in pairs(cf)do ch(self,q)end end;local dj=(di-cY-c_*0.5)/d3*d5;cX.SetPosition(cW.GetPosition()+cX.GetRotation()*dj)end;local dk=function()local dl=da()local dm,dn=d0(dl)local dp=dl+d3;local dq,dr=d0(dp)assert(dq)local di;if dn==dr or dm==cZ or dm==cY then di=dp else di=dn>=0 and cZ or cY end;df=vci.me.UnscaledTime;if di==cZ or di==cY then de=df end;db(di)end;a.NillableIfHasValueOrElse(cU.lsp,function(ds)if not a.NillableHasValue(cU.propertyName)then error('SlideSwitch: Invalid argument: propertyName is nil',3)end;local dt=a.NillableValue(cU.propertyName)da=function()return ds.GetProperty(dt,q)end;db=function(aA)ds.SetProperty(dt,aA)end;ds.AddListener(function(ao,z,du,dv)if z==dt then dh()end end)end,function()local du=q;da=function()return du end;db=function(aA)du=aA;dh()end end)self={GetColliderItem=function()return cV end,GetBaseItem=function()return cW end,GetKnobItem=function()return cX end,GetMinValue=function()return cY end,GetMaxValue=function()return cZ end,GetValue=function()return q end,SetValue=function(aA)db(d0(aA))end,GetTickFrequency=function()return d3 end,IsSnapToTick=function()return d7 end,AddListener=function(ch)cf[ch]=ch end,RemoveListener=function(ch)cf[ch]=nil end,DoUse=function()dd=true;de=vci.me.UnscaledTime;dk()end,DoUnuse=function()dd=false end,DoGrab=function()dc=true;dg=cX.GetPosition()-cW.GetPosition()end,DoUngrab=function()dc=false end,Update=function()if dc then local dw=cV.GetPosition()-cW.GetPosition()+dg;local dx=cX.GetRotation()*d5;local dy=Vector3.Project(dw,dx)local dz=(Vector3.Dot(d5,dy)>=0 and 1 or-1)*dy.magnitude/d6;local dA=(d7 and a.Round(dz)or dz)*d3+(cY+cZ)*0.5;local di=a.Clamp(dA,cY,cZ)if di~=q then db(di)end elseif dd then local dB=vci.me.UnscaledTime;if dB>=de+d8 and dB>=df+d9 then dk()end elseif cV.IsMine then a.AlignSubItemOrigin(cW,cV)end end}dh()return self end,CreateSubItemConnector=function()local dC=function(dD,cF,dE)dD.item=cF;dD.position=cF.GetPosition()dD.rotation=cF.GetRotation()dD.initialPosition=dD.position;dD.initialRotation=dD.rotation;dD.propagation=not not dE;return dD end;local dF=function(dG)for cF,dD in pairs(dG)do dC(dD,cF,dD.propagation)end end;local dH=function(dI,ba,dD,dJ,dK)local dw=dI-dD.initialPosition;local dL=ba*Quaternion.Inverse(dD.initialRotation)dD.position=dI;dD.rotation=ba;for cF,dM in pairs(dJ)do if cF~=dD.item and(not dK or dK(dM))then dM.position,dM.rotation=a.RotateAround(dM.initialPosition+dw,dM.initialRotation,dI,dL)cF.SetPosition(dM.position)cF.SetRotation(dM.rotation)end end end;local dN={}local dO=true;local dP=false;local self;self={IsEnabled=function()return dO end,SetEnabled=function(aC)dO=aC;if aC then dF(dN)dP=false end end,Contains=function(dQ)return a.NillableHasValue(dN[dQ])end,Add=function(dR,dS)if not dR then error('SubItemConnector.Add: Invalid argument: subItems = '..tostring(dR),2)end;local dT=type(dR)=='table'and dR or{dR}dF(dN)dP=false;for N,cF in pairs(dT)do dN[cF]=dC({},cF,not dS)end end,Remove=function(dQ)local aQ=a.NillableHasValue(dN[dQ])dN[dQ]=nil;return aQ end,RemoveAll=function()dN={}return true end,Each=function(ah)for cF,dD in pairs(dN)do if ah(cF,self)==false then return false end end end,GetItems=function()local dT={}for cF,dD in pairs(dN)do table.insert(dT,cF)end;return dT end,Update=function()if not dO then return end;local dU=false;for cF,dD in pairs(dN)do local dV=cF.GetPosition()local dW=cF.GetRotation()if not a.VectorApproximatelyEquals(dV,dD.position)or not a.QuaternionApproximatelyEquals(dW,dD.rotation)then if dD.propagation then if cF.IsMine then dH(dV,dW,dN[cF],dN,function(dM)if dM.item.IsMine then return true else dP=true;return false end end)dU=true;break else dP=true end else dP=true end end end;if not dU and dP then dF(dN)dP=false end end}return self end,GetSubItemTransform=function(dQ)local dI=dQ.GetPosition()local ba=dQ.GetRotation()local dX=dQ.GetLocalScale()return{positionX=dI.x,positionY=dI.y,positionZ=dI.z,rotationX=ba.x,rotationY=ba.y,rotationZ=ba.z,rotationW=ba.w,scaleX=dX.x,scaleY=dX.y,scaleZ=dX.z}end,RestoreCytanbTransform=function(dY)local dV=dY.positionX and dY.positionY and dY.positionZ and Vector3.__new(dY.positionX,dY.positionY,dY.positionZ)or nil;local dW=dY.rotationX and dY.rotationY and dY.rotationZ and dY.rotationW and Quaternion.__new(dY.rotationX,dY.rotationY,dY.rotationZ,dY.rotationW)or nil;local dX=dY.scaleX and dY.scaleY and dY.scaleZ and Vector3.__new(dY.scaleX,dY.scaleY,dY.scaleZ)or nil;return dV,dW,dX end}a.SetConstEach(a,{LogLevelOff=0,LogLevelFatal=100,LogLevelError=200,LogLevelWarn=300,LogLevelInfo=400,LogLevelDebug=500,LogLevelTrace=600,LogLevelAll=0x7FFFFFFF,ColorHueSamples=10,ColorSaturationSamples=4,ColorBrightnessSamples=5,EscapeSequenceTag='#__CYTANB',SolidusTag='#__CYTANB_SOLIDUS',NegativeNumberTag='#__CYTANB_NEGATIVE_NUMBER',ArrayNumberTag='#__CYTANB_ARRAY_NUMBER',InstanceIDParameterName='__CYTANB_INSTANCE_ID',MessageValueParameterName='__CYTANB_MESSAGE_VALUE',TypeParameterName='__CYTANB_TYPE',ColorTypeName='Color',Vector2TypeName='Vector2',Vector3TypeName='Vector3',Vector4TypeName='Vector4',QuaternionTypeName='Quaternion',LOCAL_SHARED_PROPERTY_EXPIRED_KEY='__CYTANB_LOCAL_SHARED_PROPERTY_EXPIRED'})a.SetConstEach(a,{ColorMapSize=a.ColorHueSamples*a.ColorSaturationSamples*a.ColorBrightnessSamples,FatalLogLevel=a.LogLevelFatal,ErrorLogLevel=a.LogLevelError,WarnLogLevel=a.LogLevelWarn,InfoLogLevel=a.LogLevelInfo,DebugLogLevel=a.LogLevelDebug,TraceLogLevel=a.LogLevelTrace})c={[a.ColorTypeName]={compositionFieldNames=a.ListToMap({'r','g','b','a'}),compositionFieldLength=4,toTableFunc=a.ColorToTable,fromTableFunc=a.ColorFromTable},[a.Vector2TypeName]={compositionFieldNames=a.ListToMap({'x','y'}),compositionFieldLength=2,toTableFunc=a.Vector2ToTable,fromTableFunc=a.Vector2FromTable},[a.Vector3TypeName]={compositionFieldNames=a.ListToMap({'x','y','z'}),compositionFieldLength=3,toTableFunc=a.Vector3ToTable,fromTableFunc=a.Vector3FromTable},[a.Vector4TypeName]={compositionFieldNames=a.ListToMap({'x','y','z','w'}),compositionFieldLength=4,toTableFunc=a.Vector4ToTable,fromTableFunc=a.Vector4FromTable},[a.QuaternionTypeName]={compositionFieldNames=a.ListToMap({'x','y','z','w'}),compositionFieldLength=4,toTableFunc=a.QuaternionToTable,fromTableFunc=a.QuaternionFromTable}}d={{tag=a.NegativeNumberTag,pattern='^'..a.NegativeNumberTag,replacement=''},{tag=a.ArrayNumberTag,pattern='^'..a.ArrayNumberTag,replacement=''},{tag=a.SolidusTag,pattern='^'..a.SolidusTag,replacement='/'},{tag=a.EscapeSequenceTag,pattern='^'..a.EscapeSequenceTag..a.EscapeSequenceTag,replacement=a.EscapeSequenceTag}}e=a.ListToMap({a.NegativeNumberTag,a.ArrayNumberTag})f=a.LogLevelInfo;h={[a.LogLevelFatal]='FATAL',[a.LogLevelError]='ERROR',[a.LogLevelWarn]='WARN',[a.LogLevelInfo]='INFO',[a.LogLevelDebug]='DEBUG',[a.LogLevelTrace]='TRACE'}package.loaded['cytanb']=a;i,j=(function()local c8='eff3a188-bfc7-4b0e-93cb-90fd1adc508c'local ce=_G[c8]if not ce then ce={}_G[c8]=ce end;local dZ=ce.randomSeedValue;if not dZ then dZ=os.time()-os.clock()*10000;ce.randomSeedValue=dZ;math.randomseed(dZ)end;local d_=ce.clientID;if type(d_)~='string'then d_=tostring(a.RandomUUID())ce.clientID=d_ end;local e0=vci.state.Get(b)or''if e0==''and vci.assets.IsMine then e0=tostring(a.RandomUUID())vci.state.Set(b,e0)end;return e0,d_ end)()return a end)()
