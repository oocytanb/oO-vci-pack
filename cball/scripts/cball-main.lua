...
local ToHumanReadableUnit = (function ()
    local baseIndex = 1
    local unitList = {'', 'k', 'M', 'G', 'T'}
    local unitLength = #unitList

    return function (num)
        local index = num == 0 and baseIndex or cytanb.Clamp(baseIndex + math.floor(math.log(math.abs(num), 1000)), 1, unitLength)
        return (index == baseIndex and num or num / (1000 ^ (index - baseIndex))), unitList[index]
    end
end)()
...
local OutputStats = function ()
    ...
    if statsCache.throwingPosition.y > 0 and statsCache.gravity < 0 then
        ...
        local hrDistance, hrDistanceUnit = ToHumanReadableUnit(distance)
        predictedFlyingDistanceString = cytanb.Round(hrDistance, 2) .. ' [' .. hrDistanceUnit .. 'm]'
    else
