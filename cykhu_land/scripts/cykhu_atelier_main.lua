...
local settings = (function ()
  ...
  return {
    long_press_time = TimeSpan.FromSeconds(1),
    ...
    switch_parameters = cytanb.ListToMap(
      ...
      function (entry)
        ...
        return entry.colliderName, {
          ...
          tickFrequency = 1,
          tickVector = Vector3.__new(0.0, 0.06 / (maxValue - minValue), 0.0),
        }
      end
    ),
...
local transform_status = {
  mesh = get_game_object_transform(settings.atelier_mesh_name),
...
local function set_transform_enabled(enabled, status)
  ...
  if status.bounds_item.IsMine then
    if enabled then
      ...
      status.bounds_item.SetLocalScale(
        Vector3.one * settings.bounds_scale_factor
      )
      status.bounds_item.SetPosition(hidden_position)
    end
  end
...
local update_cw; update_cw = cytanb.CreateUpdateRoutine(
  ...
  function ()
    ...
    transform_switch = assert(slide_switch_map[settings.transform_switch_name])
    ...
    cytanb.OnInstanceMessage(
      ...
      function (sender, name, parameterMap)
        if parameterMap.sender_id ~= cytanb.OwnID() then
          transform_switch.SetValue(parameterMap.transform_enabled and 1 or 0)
        end
      end
    )
